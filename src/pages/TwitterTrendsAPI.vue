  ...
  methods: {
    ...
    async beMade() {
      ...
      try {
        const res = await axios.get(
          "https://twitter-trends-redirect.herokuapp.com/api/trends"
        );
    ...
    carry() {
      ...
      this.twittertrends.forEach((value, index) => {
        ...
        const tweet_val =
          Math.abs(value.tweet_volume) > 999
            ? Math.sign(value.tweet_volume) *
                (Math.abs(value.tweet_volume) / 1000).toFixed(1) +
              "k"
            : Math.sign(value.tweet_volume) * Math.abs(value.tweet_volume);
...
<template>
  <h2 style="margin-top: 10px; margin-bottom: 0px">
    Twitter Trends (Bekasi, Indonesia)
  </h2>
  ...
  >
    ...
    <small class="tweet-volume-class">{{
      trends.tweet_volume !== 0 ? `(${trends.tweet_volume} tweets)` : ""
    }}</small>
  </div>

  <br />
  <p style="margin-top: -5px; margin-bottom: 5px; color: red">
    Platform Pengembang untuk Saya (Twitter API v2): tidak bisa trends/place API
    ❌
  </p>
  <p style="margin-top: -5px; margin-bottom: 5px; color: green">
    Platform Pengembang untuk
    <a href="https://github.com/guilhermefront/Twitter-Trending" target="_blank"
      >Github: @guilhermefront</a
    >
    (Twitter API Standard v1.1): ada trends/place API ✅
  </p>
</template>
